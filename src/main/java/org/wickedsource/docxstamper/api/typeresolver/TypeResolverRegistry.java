...
public class TypeResolverRegistry {
     ...
     */
    public <T> void registerTypeResolver(
    ...
    public <T> ITypeResolver<T> getResolverForType(Class<T> type) {
        return typeResolversByType.containsKey(type) ? (ITypeResolver<T>) typeResolversByType.get(
                type) : (ITypeResolver<T>) defaultResolver;
    }
