...
public class RunUtil {
    ...
    private static CharSequence getText(JAXBElement<?> content) {
        ...
        if (elementValue instanceof Text text) return getText(text);
        if (elementValue instanceof R.Tab) return "\t";
        return "";
    }
    ...
    private static CharSequence getText(Text text) {
        ...
        return Objects.equals(space,
                              PRESERVE) ? value // keeps spaces if spaces are to be preserved (LibreOffice seems to ignore the "space" property)
