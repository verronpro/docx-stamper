...
public class RepeatProcessor
     ...
     */
    public static ICommentProcessor newInstanceWithNullReplacement(
            PlaceholderReplacer pr
    ) {
        return new RepeatProcessor(pr,
                                   (document, row) -> RepeatProcessor.stampEmptyContext(
                                           pr,
                                           document,
                                           row));
    }
     ...
     * @return A new RepeatProcessor.
     */
    public static List<Tr> stampEmptyContext(
            PlaceholderReplacer pr,
        ...
        for (Map.Entry<Tr, List<Object>> entry : tableRowsToRepeat.entrySet()) {
            ...
            if (expressionContexts == null) {
                ...
                for (Object expressionContext : expressionContexts) {
                    ...
                    CommentWrapper commentWrapper = Objects.requireNonNull(
                            tableRowsCommentsToRemove.get(row));
                    Comments.Comment comment = Objects.requireNonNull(
                            commentWrapper.getComment());
                    BigInteger commentId = comment.getId();
                    CommentUtil.deleteCommentFromElement(rowClone.getContent(),
                                                         commentId);
                    new ParagraphResolverDocumentWalker(rowClone,
                                                        expressionContext,
                                                        document,
