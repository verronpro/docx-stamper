...
The value that an placeholder resolves to may be of the following types:
...
If an placeholder cannot be resolved successfully, it will be skipped (meaning the placeholder stays in the document as it was in the template).
...
By default, DocxStamper fails with an UnresolvedExpressionException if an placeholder within the document or within the comments cannot be resolved successfully.
